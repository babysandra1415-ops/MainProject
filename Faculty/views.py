...
def ChatList(request):
    ...
    faculty = tbl_faculty.objects.get(id=request.session["fid"])
    conversations = []
    following = tbl_follow.objects.filter(fromfaculty=faculty, follow_status=1)
    for f in following:
        if f.tostudent:
            other_id, other_type = f.tostudent_id, "student"
            name = f.tostudent.student_name
            photo = f.tostudent.student_photo
            chat_filter = (Q(faculty_from=faculty) | Q(faculty_to=faculty)) & (Q(student_from=f.tostudent) | Q(student_to=f.tostudent))
        elif f.tofaculty:
            other_id, other_type = f.tofaculty_id, "faculty"
            name = f.tofaculty.faculty_name
            photo = f.tofaculty.faculty_photo
            chat_filter = (Q(faculty_from=faculty) | Q(faculty_to=faculty)) & (Q(faculty_from=f.tofaculty) | Q(faculty_to=f.tofaculty))
        elif f.tocollege:
            other_id, other_type = f.tocollege_id, "college"
            name = f.tocollege.college_name
            photo = f.tocollege.college_photo
            chat_filter = (Q(faculty_from=faculty) | Q(faculty_to=faculty)) & (Q(college_from=f.tocollege) | Q(college_to=f.tocollege))
        else:
            continue
        last_chat = tbl_chat.objects.filter(chat_filter).order_by('-chat_time').first()
        unseen = tbl_chat.objects.filter(chat_filter, chat_seen=False).filter(faculty_to=faculty)
        conversations.append({
            "other_id": other_id, "other_type": other_type, "name": name, "photo": photo,
            "last_chat": last_chat, "unseen_count": unseen.count()
        })
    def sort_key(c):
        if c["last_chat"]:
    ...
    return render(request, "Faculty/ChatList.html", {"conversations": conversations})

def ajaxchatseen(request):
    tid = request.GET.get("tid")
    utype = request.GET.get("utype", "faculty")
    faculty = tbl_faculty.objects.get(id=request.session["fid"])
    if utype == "student":
        tbl_chat.objects.filter(student_from_id=tid, faculty_to=faculty, chat_seen=False).update(chat_seen=True)
    elif utype == "faculty":
        tbl_chat.objects.filter(faculty_from_id=tid, faculty_to=faculty, chat_seen=False).update(chat_seen=True)
    elif utype == "college":
        tbl_chat.objects.filter(college_from_id=tid, faculty_to=faculty, chat_seen=False).update(chat_seen=True)
    return JsonResponse({"ok": True})

def chatpage(request,id):
    faculty  = tbl_faculty.objects.get(id=id)
    tbl_chat.objects.filter(faculty_from_id=id, faculty_to=request.session["fid"], chat_seen=False).update(chat_seen=True)
...
def schatpage(request,id):
    ...
    tbl_chat.objects.filter(student_from_id=id, faculty_to=request.session["fid"], chat_seen=False).update(chat_seen=True)
...
def cchatpage(request,id):
    ...
    tbl_chat.objects.filter(college_from_id=id, faculty_to=request.session["fid"], chat_seen=False).update(chat_seen=True)
...
def rating(request):
    ...
    if counts>0:
        ...
        avg=res//counts
        # print(avg)
        return render(request,"Faculty/Rating.html",{'data':stardata,'ar':parray,'avg':avg,'count':counts})
    else:
         return render(request,"Faculty/Rating.html")
...
def ajaxstar(request):
    ...
    tbl_rating.objects.create(faculty=tbl_faculty.objects.get(id=request.session['fid']),user_review=user_review,rating_data=rating_data)
    stardata=tbl_rating.objects.all().order_by('-datetime')
    return render(request,"Faculty/AjaxRating.html",{'data':stardata,'ar':parray})
